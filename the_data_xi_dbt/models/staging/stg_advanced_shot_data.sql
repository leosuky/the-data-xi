...
with source_data as (
    select
        {{ dbt_utils.star(from=source('the_data_xi_raw', 'advanced_shot_data'), quote_identifiers=True) }}
    from {{ source('the_data_xi_raw', 'advanced_shot_data') }}
)
